...
class BlogController extends Controller {
	...
	public function indexAction() {
		...
		$errorList = $this->get('validator')
				->validateValue('ljk', $emailConstraint);
		foreach ($errorList AS $error) {
			$errors[] = $error;
		}
		if (count($errors) > 0) {
			return $this
					->render('AcmeBlogBundle:Blog:validate.html.twig',
							array('errors' => $errors,));
		} else {
	...
	public function listAction($_controller, $_route) {
		...
		return $this
				->render('AcmeBlogBundle:Blog:list.html.twig',
						array('result' => $posts));
	}
	...
	public function showAction() {
		...
		return $this
				->render('AcmeBlogBundle:Blog:show.html.twig',
						array('post' => $post));
	}
