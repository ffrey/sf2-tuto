...
$context = array('date' => date('Y-m-d'), 'time' => date('H:m:00'),
		'version' => $argv[1], 'api_version' => $argv[1],
		'stability' => $argv[2], 'api_stability' => $argv[2],);
...
foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path),
	...
	if (preg_match('/\.php$/', $file)) {
		...
		$context['files'] .= '        <file install-as="Swift/' . $name
				. '" name="' . $name . '" role="php" />' . "\n";
	}
}
