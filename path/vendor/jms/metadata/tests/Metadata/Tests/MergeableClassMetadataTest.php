...
class MergeableClassMetadataTest extends \PHPUnit_Framework_TestCase {
	public function testMerge() {
		$parentMetadata = new MergeableClassMetadata(
		...
		$childMetadata = new MergeableClassMetadata(
		...
		$this
				->assertEquals('Metadata\Tests\Fixtures\TestObject',
						$parentMetadata->reflection->getName());
		$this
				->assertEquals(array('foo' => 'baz', 'baz' => 'baz',),
						$parentMetadata->propertyMetadata);
		$this
				->assertEquals(array('foo' => 'baz',),
						$parentMetadata->methodMetadata);
		$this->assertEquals(1, $parentMetadata->createdAt);
		$this
				->assertEquals(array('foo', 'bar'),
						$parentMetadata->fileResources);
	}
}
