...
class PointcutMatchingPassTest extends \PHPUnit_Framework_TestCase {
	...
	public function testProcess() {
		...
		$container
				->register('pointcut',
						'JMS\AopBundle\Tests\DependencyInjection\Compiler\Fixture\LoggingPointcut')
				->addTag('jms_aop.pointcut',
						array('interceptor' => 'interceptor'));
		$container
				->register('interceptor',
						'JMS\AopBundle\Tests\DependencyInjection\Compiler\Fixture\LoggingInterceptor');
		$container
				->register('test',
						'JMS\AopBundle\Tests\DependencyInjection\Compiler\Fixture\TestService');
	...
	protected function setUp() {
		...
		if (false === @mkdir($this->cacheDir, 0777, true)) {
			throw new RuntimeException(
					sprintf('Could not create cache dir "%s".', $this->cacheDir));
		}
	}
