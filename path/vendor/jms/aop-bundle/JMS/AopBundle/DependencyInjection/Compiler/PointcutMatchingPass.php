...
class PointcutMatchingPass implements CompilerPassInterface {
	...
	public function process(ContainerBuilder $container) {
		...
		$this->cacheDir = $container->getParameter('jms_aop.cache_dir')
				. '/proxies';
		...
		foreach ($container->getDefinitions() as $id => $definition) {
			...
			$this
					->processInlineDefinitions($pointcuts, $interceptors,
		...
		$container->getDefinition('jms_aop.interceptor_loader')
				->addArgument($interceptors);
	}

	private function processInlineDefinitions($pointcuts, &$interceptors,
			array $a) {
		foreach ($a as $k => $v) {
	...
	private function processDefinition(Definition $definition, $pointcuts,
			&$interceptors) {
		...
		foreach ($class
				->getMethods(
						\ReflectionMethod::IS_PROTECTED
								| \ReflectionMethod::IS_PUBLIC) as $method) {
			if ($method->isFinal()) {
				continue;
			}
		...
		$enhancer = new Enhancer($class, array(),
				array($generator));
		$enhancer
				->writeClass(
						$filename = $this->cacheDir . '/'
								. str_replace('\\', '-', $class->name) . '.php');
	...
	private function addResources(\ReflectionClass $class) {
		do {
			$this->container
					->addResource(new FileResource($class->getFilename()));
	...
	private function getPointcuts() {
		...
		foreach ($this->container->findTaggedServiceIds('jms_aop.pointcut') as $id => $attr) {
			if (!isset($attr[0]['interceptor'])) {
				throw new RuntimeException(
						'You need to set the "interceptor" attribute for the "jms_aop.pointcut" tag of service "'
								. $id . '".');
			}
		...
		$this->container->getDefinition('jms_aop.pointcut_container')
