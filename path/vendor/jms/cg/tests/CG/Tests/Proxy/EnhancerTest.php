...
class EnhancerTest extends \PHPUnit_Framework_TestCase {
	 ...
	 */
	public function testGenerateClass($class, $generatedClass,
			array $interfaces, array $generators) {
		...
		$this
				->assertEquals(
						$this
								->getContent(
										substr($generatedClass,
												strrpos($generatedClass, '\\')
														+ 1)),
	...
	public function getGenerationTests() {
		return array(
				array('CG\Tests\Proxy\Fixture\SimpleClass',
						'CG\Tests\Proxy\Fixture\SimpleClass__CG__Enhanced',
						array('CG\Tests\Proxy\Fixture\MarkerInterface'),
						array()),
				array('CG\Tests\Proxy\Fixture\SimpleClass',
						'CG\Tests\Proxy\Fixture\SimpleClass__CG__Sluggable',
						array('CG\Tests\Proxy\Fixture\SluggableInterface'),
						array()),
				array('CG\Tests\Proxy\Fixture\Entity',
						'CG\Tests\Proxy\Fixture\Entity__CG__LazyInitializing',
						array(),
						array(new LazyInitializerGenerator(),)));
	}

	public function testInterceptionGenerator() {
		$enhancer = new Enhancer(
				new \ReflectionClass('CG\Tests\Proxy\Fixture\Entity'), array(),
				array($generator = new InterceptionGenerator()));
		$enhancer
				->setNamingStrategy(
						$this
								->getNamingStrategy(
										'CG\Tests\Proxy\Fixture\Entity__CG__Traceable_'
												. sha1(microtime(true))));
		$generator->setPrefix('');
	...
	public function testLazyInitializerGenerator() {
		$enhancer = new Enhancer(
				new \ReflectionClass('CG\Tests\Proxy\Fixture\Entity'), array(),
				array($generator = new LazyInitializerGenerator(),));
		$generator->setPrefix('');
	...
	private function getLoader(array $interceptors) {
		...
		$loader->expects($this->any())->method('loadInterceptors')
	...
	private function getNamingStrategy($name) {
		...
		$namingStrategy->expects($this->any())->method('getClassName')
