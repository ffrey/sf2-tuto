...
class PhpClassTest extends \PHPUnit_Framework_TestCase {
	public function testFromReflection() {
		$class = new PhpClass();
		$class->setName('CG\Tests\Generator\Fixture\Entity')->setAbstract(true)
				->setDocblock(
						'/**
 * Doc Comment.
 *
 ...
 */')
				->setProperty(
						PhpProperty::create('id')->setVisibility('private')
								->setDocblock('/**
 * @var integer
 */'))
				->setProperty(
						PhpProperty::create('enabled')
								->setVisibility('private')
								->setDefaultValue(false));

		$method = PhpMethod::create()->setName('__construct')->setFinal(true)
				->addParameter(new PhpParameter('a'))
				->addParameter(
						PhpParameter::create()->setName('b')->setType('array')
								->setPassedByReference(true))
				->addParameter(
						PhpParameter::create()->setName('c')
								->setType('stdClass'))
				->addParameter(
						PhpParameter::create()->setName('d')
								->setDefaultValue('foo'))
				->setDocblock(
						'/**
 * Another doc comment.
 ...
 */');
		...
		$class
				->setMethod(
						PhpMethod::create()->setName('foo')->setAbstract(true)
								->setVisibility('protected'));

		$class
				->setMethod(
						PhpMethod::create()->setName('bar')->setStatic(true)
								->setVisibility('private'));

		$this
				->assertEquals($class,
						PhpClass::fromReflection(
								new \ReflectionClass(
										'CG\Tests\Generator\Fixture\Entity')));
	}
	...
	public function testSetGetConstants() {
		...
		$this
				->assertEquals(array('foo' => 'bar', 'bar' => 'baz'),
						$class->getConstants());
	...
	public function testSetGetProperties() {
		...
		$this
				->assertSame($class,
						$class->setProperty($prop = new PhpProperty('foo')));
	...
	public function testSetGetMethods() {
		...
		$this
				->assertSame($class,
						$class->setMethod($method = new PhpMethod('foo')));
